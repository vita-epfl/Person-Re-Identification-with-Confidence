import train_imgreid_xent_htri_custom as xent_script
import sys
import argparse
...
def main(args):
    ...
    stepSize_choice = ["--stepsize", "30", "60"]
    gpu_choice = ["--gpu-devices", args.gpu_devices]
    #weight_loading = ["--load-weights","pretrained_models/resnet50_xent_market1501.pth.tar"]
    weight_loading = ["--load-weights",args.load_weights]
    eval_steps = ["--eval-step", "10"]
    enable_scheduler = ["--scheduler","1"]
    fixBase = [] if args.htri_only else ["--fixbase-epoch","6" ,"--fixbase-lr","0.0003"]
    margin = ["--margin", str(args.margin)]
    multiplier = args.multiplier
    htri_only = [] if not args.htri_only else ["--htri-only"]
    soft_margin = [] if not args.soft_margin else ["--soft-margin"]
    ...
    lr_search = ["0.0005","0.0003","0.00035","0.0004","0.0005","0.0006","0.0007","0.001"]

    aa = math.log(0.0009, 10)
    bb = math.log(0.002, 10)
    log_lr_list = np.random.uniform(aa, bb, int(5*multiplier)).tolist()
    #lr_search = [10**xx for xx in log_lr_list]
    # Beta
    #xent_list = np.random.uniform(0.9, 3, int(5)).tolist()*multiplier
    xent_list = ["1.7","0.0003","0.00035","0.0004","0.0005","0.0006","0.0007","0.001"]
    #htri_list = np.random.uniform(0.5, 7, int(5)).tolist()*multiplier
    htri_list = [1,1,1,1,1]*multiplier

    saved_folders = []
    final_results = []

    writer_dict={}
    writer = SummaryWriter(log_dir=osp.join(src,'Accuracy_vs_LR_xent_htri',"_".join([args.arch,data_choice[1],optim_choice[1], "e"+epoch_choice[1], "b"+train_batch[1],"{}lr".format(5*multiplier),args.suffix]), 'tensorboard'))
    ...
    for idx,lr_elem in enumerate(lr_search):
        try:
            ...
            htri = htri_list[idx]
            htri_choice = ["--lambda-htri", str(htri)]
            folder_save = osp.join(src,'log_resnet50_htri', "_".join([args.arch,data_choice[1],optim_choice[1], "e"+epoch_choice[1], "b"+train_batch[1],"lr"+lr_choice[1], "xentLambda"+str(xent), "htriLambda"+str(htri),'NotPretrained',args.suffix]))

            save_dir = ["--save-dir",folder_save]
            saved_folders.append(folder_save)
            arg_list.extend(root+ arch_choice+data_choice+optim_choice+epoch_choice+train_batch+test_batch+height_width+stepSize_choice+gpu_choice+eval_steps+lr_choice+label_smooth+save_dir+fixBase+xent_choice+htri_choice+htri_only+margin+soft_margin)
