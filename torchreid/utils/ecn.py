...
import pdb
import torch
def ECN_custom(queryset,testset,k,t,q,method, use_cosine=False):
    ...
    ntest=testset.shape[0]
    total= nQuery + ntest
    mat= torch.cat((queryset,testset),0)
    if use_cosine:
        mat = mat/mat.norm(dim=1)[:,None]
        r_dist = torch.addmm(1,torch.ones((total,total)),-1,mat,mat.transpose(0,1))
    else:
        r_dist = torch.pow(mat, 2).sum(dim=1, keepdim=True).expand(total, total) + \
                  torch.pow(mat, 2).sum(dim=1, keepdim=True).expand(total, total).t()
        r_dist.addmm_(1, -2, mat, mat.t())
    r_dist = r_dist.numpy()
